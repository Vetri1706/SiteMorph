...
function createDocument({ climate, site, requirements, building, trace, assets, siteFitAssessment, selectedSiteFitOptionId }: ReportInput): Document {
  ...
  children.push(callout([
    ...
    richParagraph([run("Requested concept: ", { bold: true }), run(`${concept}  |  ${formatNumber(requirements.totalAreaSqFt || requirements.targetFootprintSqFt)} ft2`)], { after: 0 }),
  ], PALE_TEAL, TEAL));
  ...
  children.push(matrixTable(["Key signal", "Current finding"], [
    ...
    ["Design status", building ? `Forma Design R${building.revision}` : "Building not generated"],
  ], [3600, 5760]));
  children.push(callout([
  ...
  children.push(callout(building ? [
