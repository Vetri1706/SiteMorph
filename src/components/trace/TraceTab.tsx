...
export function TraceTab() {
  ...
  const exportReport = async () => {
    ...
    try {
      ...
      const report = await buildSiteIntelligenceReport({ climate, site, requirements, building: generatedBuilding, trace, assets });
    ...
    <div className="tab-content trace-tab">
      ...
      <Section>
        ...
        <div className="export-list">
          ...
          <div><Send size={16} /><span><strong>Native Forma proposal → Revit</strong><small>Persisted, placement-verified proposal sent through Autodesk’s Revit add-in workflow</small></span><b>FORMA</b></div>
        ...
        {revitHandoff && <div className="revit-handoff-guide"><strong><CheckCircle2 size={15} />Continue in Autodesk Forma</strong><ol>{revitHandoff.instructions.map((instruction) => <li key={instruction}>{instruction}</li>)}</ol></div>}
        ...
        <p className="button-note">The Forma proposal is the native Revit handoff. JSON is optional evidence, and OBJ is generic reference geometry.</p>
      </Section>
    </div>
  );
}
